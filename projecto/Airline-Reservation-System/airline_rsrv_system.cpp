...
{				
	...
	public:
		...
		/* --BOOK SEATS--*/
		static void resSeat(int num, int val); //// fluctúa los asientos reservados de un vuelo
...
void Flight::addFlight(){ 
	...
	LOOP:do{
		...
		if ( (temp.length() <= 10) && (checkString(temp)) ){
			this -> from = temp;//le das el lugar que quieras mano
			flag = true;
		}else {
	...
	LOOP2:do{
		getline(cin, temp);
		if ( (temp.length() <= 30) && (checkString(temp)) && (temp.compare(this -> from)) ){
			this -> to = temp;
			flag = true;
		}else{
	...
	LOOP3:do{
		...
		while(pch != NULL){
			...
			//en el vector fields primero abajo pone la
			//hora luego pone los minutos encima 
			pch = strtok(NULL, ":");
		}
		...
		if ((hour >=0 && hour<=23) && (min>=0 && min <=59)){ 
			this -> t_leave.hour = hour; //pone enteros en la estructura
			this -> t_leave.min = min; //Time que tiene como atributos la clase Flight
			flag = true;
		}else{
			cout << "Please insert a valid boarding time (e.g. 19:40)! ";
			fields.clear();		
	...
	flag = false;
	//clear fields porque ya metiste
	//en el objeto Fligh en
...
void Flight::displayFlightInfo(int num){
	...
	for (std::list<Flight>::iterator i = flist.begin(); i != flist.end(); ++i) {
		if (num == i -> flightNo) {
			...
			cout << left << setw(10) << i -> flightNo <<
					left << setw(10) << i -> from << 
					left << setw(10) << i -> to << 
					left << setw(10) << l_time << 
					left << setw(10) << a_time << 
					left << setw(10) << i -> cost << 
					left << setw(10) << i -> plane_type << 
					left << setw(10) << i -> seats << 
					left << setw(10) << i -> booked_seats << endl;
...
//num:Nro Flight, y val 
//es el numero de asientos en reserva que aumenta
...
void Person::book(){
	...
	if (!flist.empty()) {
		...
		*/
		...
		//DIRECT FLIGHT
		if(Flight::checkForSeats(num)){ // si hay hacientos libres, sino no permite		
